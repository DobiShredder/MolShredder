...
Trajectory command는 active molecular object's topology에 DCD/TRR/XTC coordinate source를 붙이고,
...
molshredder analyze trajectory rmsd|rmsf [--selection EXPR]
                                        ...
                                        [--precision 0..15]
                                        [--unit angstrom|nanometer]
molshredder traj load --path PATH [--file-format auto|dcd|trr|xtc]
...
전에 거부한다. `auto`는 case-insensitive `.dcd`, `.trr`, `.xtc` suffix를 사용하고 알 수 없는 suffix는
추정하지 않는다. 기본 cache budget은 256 MiB이며 decoded scientific payload의 상한이지 process RSS
hard limit은 아니다.
...
보존한다. Core에는 elapsed-time conversion이 있지만 실제 Qt timer/event-loop scheduler는 아직 없다.
...
- Qt timer/event-loop scheduler와 stop/rewind convenience action
