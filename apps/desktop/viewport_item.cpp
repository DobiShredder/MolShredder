...
QString outcome_error(const application::DispatchOutcome& outcome) {
  const auto* error =
      std::get_if<operation::Error>(&outcome.envelope.payload);
...
QString playback_mode_name(trajectory::PlaybackMode mode) {
  if (mode == trajectory::PlaybackMode::loop) return QStringLiteral("loop");
  if (mode == trajectory::PlaybackMode::rock) return QStringLiteral("rock");
...
render::RenderPacket demo_packet() {
  ...
  for (std::size_t index = 0; index < residue_count; ++index) {
    ...
    if (!residue.has_value()) return {};
    const auto ca = builder.add_atom(model::AtomRecord{
        "CA", 6U, residue.value(), "", 0, std::nullopt});
    const auto oxygen = builder.add_atom(model::AtomRecord{
        "O", 8U, residue.value(), "", 0, std::nullopt});
    if (!ca.has_value() || !oxygen.has_value()) return {};
    if (builder
  ...
  if (!topology.has_value() || !frame.has_value()) return {};
...
void build_sphere_geometry(std::vector<SolidVertexGpu>& vertices,
  ...
  for (std::size_t latitude = 0; latitude <= latitude_count; ++latitude) {
    ...
    for (std::size_t longitude = 0; longitude <= longitude_count;
         ++longitude) {
      const auto phi = 2.0F * std::numbers::pi_v<float> *
                       static_cast<float>(longitude) /
                       static_cast<float>(longitude_count);
      const std::array<float, 3> normal{
          ring_radius * std::cos(phi), y, ring_radius * std::sin(phi)};
      vertices.push_back(SolidVertexGpu{
  ...
  for (std::size_t latitude = 0; latitude < latitude_count; ++latitude) {
    for (std::size_t longitude = 0; longitude < longitude_count;
         ++longitude) {
      const auto first =
          static_cast<std::uint32_t>(latitude * row + longitude);
      const auto second = static_cast<std::uint32_t>((latitude + 1U) * row +
                                                     longitude);
      indices.insert(indices.end(),
                     {first, second, first + 1U, first + 1U, second,
                      second + 1U});
    }
  }
}
...
void build_cylinder_geometry(std::vector<SolidVertexGpu>& vertices,
  ...
  for (std::size_t segment = 0; segment < segment_count; ++segment) {
    ...
    indices.insert(indices.end(),
                   {first, first + 1U, first + 2U, first + 2U, first + 1U,
                    first + 3U});
  }
  const auto bottom_center = static_cast<std::uint32_t>(vertices.size());
  vertices.push_back(SolidVertexGpu{{0.0F, 0.0F, 0.0F},
                                    {0.0F, 0.0F, -1.0F}});
  ...
  for (std::size_t segment = 0; segment <= segment_count; ++segment) {
    const auto angle = 2.0F * std::numbers::pi_v<float> *
                       ...
                       static_cast<float>(segment_count);
    vertices.push_back(SolidVertexGpu{
        {std::cos(angle), std::sin(angle), 0.0F}, {0.0F, 0.0F, -1.0F}});
  }
  const auto top_center = static_cast<std::uint32_t>(vertices.size());
  vertices.push_back(
      SolidVertexGpu{{0.0F, 0.0F, 1.0F}, {0.0F, 0.0F, 1.0F}});
  ...
  for (std::size_t segment = 0; segment <= segment_count; ++segment) {
    const auto angle = 2.0F * std::numbers::pi_v<float> *
                       ...
                       static_cast<float>(segment_count);
    vertices.push_back(SolidVertexGpu{
        {std::cos(angle), std::sin(angle), 1.0F}, {0.0F, 0.0F, 1.0F}});
  }
...
class MolecularViewportRenderer final : public QQuickRhiItemRenderer {
 public:
  MolecularViewportRenderer() {
    line_corners_ = {{0.0F, -0.5F}, {1.0F, -0.5F},
                     {0.0F, 0.5F},  {1.0F, 0.5F}};
  ...
  void initialize(QRhiCommandBuffer* command_buffer) override {
    ...
    if (!uniform_buffer_) create_uniform_resources();
    ...
    if (geometry_dirty_) upload_geometry(command_buffer);
  ...
  void synchronize(QQuickRhiItem* item) override {
    ...
    if (revision_ == viewport->packetRevision()) return;
    ...
    for (const auto& [source_id, unused_target] : packet.pick_targets) {
      ...
      const auto gpu_id =
          static_cast<std::uint32_t>(pick_source_ids->size());
    ...
    for (const auto& vertex : packet.mesh_vertices) {
      mesh_vertices_.push_back(MeshVertexGpu{
    ...
    for (const auto& triangle : packet.mesh_triangles) {
      ...
      for (const auto vertex_index :
        ...
        pick_mesh_vertices_.push_back(PickMeshVertexGpu{
            {static_cast<float>(vertex.position.x),
             static_cast<float>(vertex.position.y),
             static_cast<float>(vertex.position.z)},
    ...
    for (const auto& line : packet.lines) {
      line_instances_.push_back(LineInstanceGpu{
          ...
          0.0F,
          {line.start_color.red, line.start_color.green,
           line.start_color.blue, line.start_color.alpha},
    ...
    for (const auto& cylinder : packet.cylinders) {
      cylinder_instances_.push_back(CylinderInstanceGpu{
          ...
          {pick_color(cylinder.pick_id)[0], pick_color(cylinder.pick_id)[1],
           pick_color(cylinder.pick_id)[2],
           pick_color(cylinder.pick_id)[3]}});
    }
  ...
  void render(QRhiCommandBuffer* command_buffer) override {
    if (!uniform_buffer_ || !bindings_) return;
    if (geometry_dirty_) upload_geometry(command_buffer);
    update_view_projection();
    auto* updates = rhi_->nextResourceUpdateBatch();
    QMatrix4x4 model;
    model.translate(static_cast<float>(center_.x), static_cast<float>(center_.y),
    ...
    command_buffer->beginPass(
        renderTarget(), QColor::fromRgbF(0.018F, 0.025F, 0.04F), {1.0F, 0U},
        updates);
    ...
    if (pick_pending_) render_pick(command_buffer);
    if (!primitive_logged_ && !mesh_indices_.empty() &&
      ...
      qInfo("MolShredder GPU primitives ready: mesh=%llu lines=%llu cylinders=%llu spheres=%llu",
 ...
 private:
  ...
  void ensure_pick_target(const QSize& size) {
    if (size.isEmpty() || pick_target_size_ == size) return;
    reset_pick_pipelines();
    pick_render_target_.reset();
    pick_render_pass_.reset();
    pick_depth_buffer_.reset();
    ...
    pick_render_target_.reset(rhi_->newTextureRenderTarget(
  ...
  void create_uniform_resources() {
    uniform_buffer_.reset(rhi_->newBuffer(
        QRhiBuffer::Dynamic, QRhiBuffer::UniformBuffer, 144U));
    ...
    if (!bindings_->create()) bindings_.reset();
  }
  ...
  void create_pipelines() {
    ...
    if (!mesh_pipeline_) {
      ...
      if (!mesh_pipeline_->create()) mesh_pipeline_.reset();
    }
    if (!sphere_pipeline_) {
      ...
      if (!sphere_pipeline_->create()) sphere_pipeline_.reset();
    }
    if (!cylinder_pipeline_) {
      ...
      if (!cylinder_pipeline_->create()) cylinder_pipeline_.reset();
    }
    if (!line_pipeline_) {
      ...
      line_pipeline_->setShaderStages({
          {QRhiShaderStage::Vertex,
           shader(QLatin1String(
               ":/molshredder/desktop/shaders/line.vert.qsb"))},
          {QRhiShaderStage::Fragment,
           shader(QLatin1String(
      ...
      if (!line_pipeline_->create()) line_pipeline_.reset();
    }
    if (!pick_render_pass_) return;
    if (!pick_mesh_pipeline_) {
      ...
      if (!pick_mesh_pipeline_->create()) pick_mesh_pipeline_.reset();
    }
    if (!pick_sphere_pipeline_) {
      ...
      if (!pick_sphere_pipeline_->create()) pick_sphere_pipeline_.reset();
    }
    if (!pick_cylinder_pipeline_) {
      ...
      if (!pick_cylinder_pipeline_->create()) pick_cylinder_pipeline_.reset();
    }
    if (!pick_line_pipeline_) {
      ...
      if (!pick_line_pipeline_->create()) pick_line_pipeline_.reset();
    }
  }
  ...
  void upload_geometry(QRhiCommandBuffer* command_buffer) {
    ...
    const auto pick_mesh_ok = upload_vector(
        pick_mesh_vertex_buffer_, QRhiBuffer::VertexBuffer,
    ...
    if (!pick_mesh_ok) pick_mesh_vertex_buffer_.reset();
    if (!sphere_ok) sphere_instance_buffer_.reset();
    if (!cylinder_ok) cylinder_instance_buffer_.reset();
    if (!line_ok) line_instance_buffer_.reset();
  ...
  void draw_spheres(QRhiCommandBuffer* command_buffer) {
    ...
    command_buffer->drawIndexed(
        static_cast<quint32>(sphere_indices_.size()),
  ...
  void draw_lines(QRhiCommandBuffer* command_buffer) {
    ...
    command_buffer->setVertexInput(0, static_cast<int>(inputs.size()),
                                   ...
                                   QRhiCommandBuffer::IndexUInt32);
    command_buffer->drawIndexed(
        static_cast<quint32>(line_indices_.size()),
  ...
  void render_pick(QRhiCommandBuffer* command_buffer) {
    ...
    const auto x = std::clamp(
        static_cast<int>(std::floor(pick_position_.x() /
                                    pick_item_size_.width() *
                                    static_cast<double>(pick_target_size_.width()))),
        0, pick_target_size_.width() - 1);
    const auto y = std::clamp(
        static_cast<int>(std::floor(pick_position_.y() /
                                    pick_item_size_.height() *
    ...
    state->result.completed =
        [state, viewport, source_ids, request_revision, packet_revision] {
          ...
          if (viewport != nullptr) {
            QMetaObject::invokeMethod(
                ...
                [viewport, request_revision, packet_revision, source_id] {
                  if (viewport != nullptr) {
                    viewport->deliverPickResult(request_revision,
                                                packet_revision, source_id);
                  }
                },
                Qt::QueuedConnection);
          }
    ...
    updates->readBackTexture(QRhiReadbackDescription{pick_readback_texture_.get()},
                             &state->result);
    command_buffer->resourceUpdate(updates);
  }
...
void MolecularViewport::setAngle(float angle) {
  if (qFuzzyCompare(angle_, angle)) return;
  angle_ = angle;
  emit angleChanged();
  update();
}
...
bool MolecularViewport::loadStructure(const QUrl& url) {
  if (!url.isLocalFile()) {
    setStatus(QStringLiteral("Only local PDB/mmCIF files are supported"),
  ...
  atom_count_ = static_cast<qulonglong>(object->system->topology()->atom_count());
  selection_text_ = QStringLiteral("No selection");
  emit selectionChanged();
  if (!rebuildRepresentation()) return false;
  syncTrajectoryState();
  resetView();
  qInfo("MolShredder desktop loaded: atoms=%llu representation=%s primitives=%llu",
...
bool MolecularViewport::loadTrajectory(const QUrl& url) {
  if (!url.isLocalFile()) {
    setStatus(QStringLiteral("Only local DCD/XTC/TRR trajectories are supported"),
              atom_count_, primitive_count_);
    return false;
  }
  ...
  if (!invokeTrajectoryAction(
          "traj load", {{"path", file_info.filesystemFilePath().string()}},
...
bool MolecularViewport::seekTrajectory(qulonglong frame) {
  if (!has_trajectory_ || frame >= trajectory_frame_count_) return false;
...
bool MolecularViewport::setTrajectoryPlaying(bool playing) {
  if (!has_trajectory_) return false;
  if (!playing) {
    return invokeTrajectoryAction("traj pause", {},
                                  QStringLiteral("Trajectory pause failed: "),
                                  false);
  }
  const auto started = invokeTrajectoryAction(
      "traj play",
      {{"mode", playback_mode_.toStdString()},
       {"direction", playback_direction_.toStdString()}, {"steps", "0"}},
...
bool MolecularViewport::stepTrajectory(int direction) {
  if (!has_trajectory_ || direction == 0) return false;
  ...
  if (!invokeTrajectoryAction(
          "traj play",
          {{"mode", playback_mode_.toStdString()},
           {"direction", selected_direction}, {"steps", "1"}},
          QStringLiteral("Trajectory step failed: "), true)) {
    return false;
  }
  return invokeTrajectoryAction("traj pause", {},
                                QStringLiteral("Trajectory pause failed: "),
...
bool MolecularViewport::setPlaybackMode(const QString& mode) {
  ...
  if (!has_trajectory_ ||
      (normalized != QStringLiteral("once") &&
  ...
  if (!invokeTrajectoryAction(
          ...
          {{"mode", normalized.toStdString()},
           {"direction", playback_direction_.toStdString()}, {"steps", "0"}},
          QStringLiteral("Playback mode failed: "), false)) {
    return false;
  }
  if (!was_playing) {
    return invokeTrajectoryAction("traj pause", {},
...
bool MolecularViewport::setPlaybackDirection(const QString& direction) {
  ...
  if (!has_trajectory_ ||
      (normalized != QStringLiteral("forward") &&
  ...
  if (!invokeTrajectoryAction(
          "traj play",
          {{"mode", playback_mode_.toStdString()},
           {"direction", normalized.toStdString()}, {"steps", "0"}},
          QStringLiteral("Playback direction failed: "), false)) {
    return false;
  }
  if (!was_playing) {
    return invokeTrajectoryAction(
        "traj pause", {}, QStringLiteral("Playback direction failed: "),
        false);
  }
  return true;
}
...
bool MolecularViewport::activateObject(qulonglong object_id) {
  ...
  const auto rebuilt = object->representations.empty()
...
void MolecularViewport::pan(double delta_x, double delta_y) {
  if (!camera_.has_value() || height() <= 0.0) return;
...
void MolecularViewport::resetView() {
  ...
  if (width() > 0.0 && height() > 0.0) {
    ...
    if (with_viewport.has_value()) camera = std::move(with_viewport);
  }
  auto framed = camera.value().frame_sphere(center, radius, 1.35);
  if (!framed.has_value()) return;
  camera_ = std::move(framed.value());
  qInfo("MolShredder camera framed: center=(%.3f,%.3f,%.3f) radius=%.3f distance=%.3f aspect=%.3f",
...
void MolecularViewport::deliverPickResult(std::uint64_t request_revision,
  ...
  const auto* object = target.has_value()
                           ? workspace_->object_by_scene_node(
  ...
  if (object != nullptr && workspace_->active_object() != nullptr &&
    ...
    if (!activated.succeeded()) {
      selection_text_ = QStringLiteral("Pick activation failed: ") +
                        outcome_error(activated);
      emit selectionChanged();
      return;
    }
  ...
  if (target.has_value() && topology != nullptr &&
    ...
    if (target->kind == render::PickKind::atom && target->atom.has_value() &&
      ...
      description =
          QStringLiteral("Atom %1 · %2 · %3 %4%5").arg(
              static_cast<qulonglong>(index + 1U))
  ...
  if (object != nullptr) {
    ...
    if (!atom_indices.empty()) {
      ...
      for (const auto index : atom_indices) {
        if (!expression.empty()) expression += " or ";
    ...
    if (!selected.succeeded()) {
      description = QStringLiteral("Pick selection failed: ") +
                    outcome_error(selected);
  ...
  qInfo("MolShredder GPU pick ready: kind=%s id=%llu selection=%s",
        ...
        static_cast<unsigned long long>(pick_id),
...
bool MolecularViewport::rebuildRepresentation() {
  ...
  action.parameters.emplace("representation",
                            representation_.toStdString());
  ...
  qInfo("MolShredder desktop representation: atoms=%llu representation=%s primitives=%llu",
        static_cast<unsigned long long>(atom_count_),
        representation_.toUtf8().constData(),
        static_cast<unsigned long long>(primitive_count_));
  return true;
...
void MolecularViewport::syncActiveRepresentationName() {
  const auto* object = workspace_->active_object();
  if (object == nullptr || object->representations.empty()) return;
  const auto name = representation_name(object->representations.back().kind);
  if (representation_ == name) return;
...
bool MolecularViewport::rebuildScenePacket() {
  ...
  for (const auto& object : workspace_->objects()) {
    if (!workspace_->scene()->effectively_visible(object.scene_node)) continue;
  ...
  setStatus(QString::fromStdString(active->system->name()) +
                QStringLiteral(" · ") + representation_ +
                QStringLiteral(" · ") + QString::number(atom_count_) +
...
void MolecularViewport::syncTrajectoryState() {
  ...
  const auto changed = has_trajectory_ != has_trajectory ||
  ...
  if (trajectory_playing_) {
    if (!playback_elapsed_.isValid()) playback_elapsed_.start();
    if (!playback_timer_.isActive()) playback_timer_.start();
  } else {
    playback_timer_.stop();
    playback_elapsed_.invalidate();
  }
  if (changed) emit trajectoryChanged();
}
...
void MolecularViewport::onPlaybackTick() {
  ...
  if (!tickTrajectory(static_cast<double>(elapsed_ms))) {
    static_cast<void>(invokeTrajectoryAction(
        "traj pause", {}, QStringLiteral("Trajectory pause failed: "),
        false));
  }
}
...
bool MolecularViewport::invokeTrajectoryAction(
  ...
  if (rebuild_scene && !rebuildScenePacket()) return false;
  syncTrajectoryState();
  return true;
}
