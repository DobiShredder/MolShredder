...
enum class NodeKind { root, group, molecular_system };
...
class SceneNode {
 public:
  ...
  [[nodiscard]] std::optional<NodeId> parent() const noexcept { return parent_; }
  ...
  [[nodiscard]] const std::shared_ptr<const model::MolecularSystem>& system()
      const noexcept {
    return system_;
  }
...
class SceneBuilder {
 public:
  ...
  [[nodiscard]] operation::Result<NodeId> add_group(
      NodeId parent, std::string name, Transform transform = {});
  [[nodiscard]] operation::Result<NodeId> add_system(
  ...
  [[nodiscard]] std::optional<operation::Error> set_visible(NodeId id,
                                                            bool visible);
  [[nodiscard]] std::optional<operation::Error> set_transform(
      NodeId id, Transform transform);
  [[nodiscard]] std::optional<operation::Error> replace_system(
      NodeId id, std::shared_ptr<const model::MolecularSystem> system);
  [[nodiscard]] std::optional<operation::Error> reparent(
  ...
  [[nodiscard]] std::optional<operation::Error> set_selection(
 ...
 private:
  ...
  [[nodiscard]] operation::Result<NodeId> add_node(
