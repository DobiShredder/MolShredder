...
double numeric(const molshredder::command::Value& value) {
  if (const auto* plain = std::get_if<double>(&value.data)) return *plain;
...
bool wait_for_prefetch(
  ...
  for (std::size_t attempt = 0; attempt < 200U; ++attempt) {
    if (scheduler->snapshot().state == state) return true;
...
int main(int argc, char** argv) {
  ...
  if (argc != 4) {
    std::cerr << "expected PDB fixture, temporary directory, and H-bond fixture\n";
    return 2;
  }
  bool passed = true;
  ...
  passed &= expect(
      trigger(gui, "load", {{"path", pdb.string()}, {"name", "protein"}})
              .succeeded() &&
          trigger(gui, "show",
  ...
  const auto selected =
      trigger(gui, "traj frame", {{"frame", "2"}});
  ...
  const auto* center_response = center.succeeded()
                                    ? std::get_if<command::Response>(
                                          &center.envelope.payload)
                                    : nullptr;
  const auto* center_position =
      center_response != nullptr
          ...
          : nullptr;
  passed &= expect(
      center_position != nullptr && center_position->size() == 3U &&
  ...
  const auto center_series = trigger(
      gui, "analyze trajectory center",
      {{"selection", "all"}, {"mode", "centroid"}, {"first", "0"},
       {"last", "2"}, {"stride", "2"}, {"missing", "error"},
  ...
  passed &= expect(
      center_series_response != nullptr &&
          ...
          workspace->active_object()->trajectory->timeline.snapshot().frame ==
              2U,
      "center series must produce a typed strided table without seeking the viewport");
  const auto center_csv = center_series_response != nullptr
                              ? command::render(
                                    center_series.envelope,
  ...
  passed &= expect(
      center_csv.has_value() &&
          ...
          center_csv.value().find("0,100,,,1,11,21") !=
              std::string::npos,
  ...
  passed &= expect(
      ...
      "GUI analysis presentation must expose the same table and CSV without a scalar marker");

  const auto distance_series = trigger(
      gui, "analyze trajectory distance",
      {{"from", "index 1"}, {"to", "index 3"}, {"pbc", "raw"},
  ...
  const auto* distance_series_response =
      distance_series.succeeded()
          ...
          : nullptr;
  passed &= expect(
      distance_series_response != nullptr &&
          distance_series_response->table.has_value() &&
          distance_series_response->table->rows.size() == 3U &&
          numeric(distance_series_response->table->rows[0][9]) == 0.34641 &&
          workspace->measurements().empty(),
      "distance series must use the shared kernel without creating persistent measurements");
  const auto contact_occupancy=trigger(gui,"analyze trajectory contacts",
      {{"selection1","index 1"},{"selection2","index 3"},{"cutoff","4"},
       {"pbc","raw"},{"exclude-bonded","false"},{"report","occupancy"},
       {"first","0"},{"last","2"},{"stride","1"},{"precision","6"},
       {"unit","angstrom"}});
  const auto* contact_occupancy_response=contact_occupancy.succeeded()
      ? std::get_if<command::Response>(&contact_occupancy.envelope.payload) : nullptr;
  passed &= expect(contact_occupancy_response!=nullptr &&
      contact_occupancy_response->table.has_value() &&
      contact_occupancy_response->table->rows.size()==1U &&
      std::get<std::uint64_t>(contact_occupancy_response->table->rows[0][2].data)==3U &&
  ...
  const auto contact_presentation=gui::make_analysis_presentation(contact_occupancy);
  passed &= expect(contact_presentation.has_value() &&
      contact_presentation.value().title=="Trajectory contact analysis" &&
      contact_presentation.value().table.has_value(),
      "GUI presenter must expose trajectory contact occupancy");

  const auto rmsd_series = trigger(
      gui, "analyze trajectory rmsd",
      {{"selection", "all"}, {"reference", "0"}, {"first", "0"},
       {"last", "2"}, {"stride", "1"}, {"fit", "none"},
       {"weight", "uniform"}, {"missing", "error"},
       {"precision", "6"}, {"unit", "angstrom"}});
  ...
  passed &= expect(
      rmsd_response != nullptr && rmsd_response->table.has_value() &&
          ...
          std::get<std::string>(rmsd_response->fields.at("fit_selection").data) ==
              "all" &&
          workspace->active_object()->trajectory->timeline.snapshot().frame ==
              2U,
      "RMSD command must score raw frames against a reference without moving the viewport");
  const auto rmsd_presentation = gui::make_analysis_presentation(rmsd_series);
  passed &= expect(rmsd_presentation.has_value() &&
                       rmsd_presentation.value().table.has_value() &&
                       rmsd_presentation.value().title ==
                           "Trajectory RMSD analysis",
                   "GUI presenter must expose the shared RMSD table");
  const auto mass_rmsd = trigger(
      gui, "analyze trajectory rmsd",
      {{"selection", "all"}, {"reference", "0"}, {"first", "0"},
       {"last", "0"}, {"fit", "none"}, {"weight", "mass"}});
  ...
  passed &= expect(
      mass_rmsd_response != nullptr &&
          std::get<std::string>(
              mass_rmsd_response->fields.at("weight").data) == "mass" &&
          std::get<bool>(
  ...
  const auto rmsf_series = trigger(
      gui, "analyze trajectory rmsf",
      {{"selection", "all"}, {"reference", "0"}, {"first", "0"},
       {"last", "2"}, {"stride", "1"}, {"fit", "none"},
       {"weight", "uniform"}, {"missing", "error"},
       {"precision", "6"}, {"unit", "angstrom"}});
  ...
  passed &= expect(
      rmsf_response != nullptr && rmsf_response->table.has_value() &&
          ...
          std::get<std::uint64_t>(
              rmsf_response->table->rows[0][8].data) == 3U,
      "RMSF command must return per-atom population fluctuation and observations");
  passed &= expect(
  ...
  passed &= expect(trigger(hbond_gui,"load",{{"path",hbond_pdb.string()},
      {"name","trajectory_hbond"}}).succeeded() &&
      trigger(hbond_gui,"traj load",{{"path",trajectory_path.string()},
      {"file-format","dcd"},{"cache-mib","1"},{"prefetch-frames","0"}}).succeeded(),
      "trajectory H-bond fixture must load");
  const auto hbond_occupancy=trigger(hbond_gui,"analyze trajectory hbonds",
      {{"donors","chain A"},{"acceptors","chain B"},{"cutoff","4"},
       {"angle","30"},{"pbc","raw"},{"report","occupancy"},
       {"first","0"},{"last","2"},{"stride","1"},{"precision","6"},
       {"unit","angstrom"}});
  const auto* hbond_occupancy_response=hbond_occupancy.succeeded()
      ? std::get_if<command::Response>(&hbond_occupancy.envelope.payload) : nullptr;
  passed &= expect(hbond_occupancy_response!=nullptr &&
      hbond_occupancy_response->table.has_value() &&
      hbond_occupancy_response->table->rows.size()==1U &&
      std::get<std::uint64_t>(hbond_occupancy_response->table->rows[0][3].data)==3U &&
      numeric(hbond_occupancy_response->table->rows[0][4])==1.0 &&
      std::get<std::string>(hbond_occupancy_response->fields.at("donor_typing_source").data)=="element-bond-v1",
      "trajectory H-bond command must expose triple occupancy and typing provenance");
  const auto hbond_presentation=gui::make_analysis_presentation(hbond_occupancy);
  passed &= expect(hbond_presentation.has_value() &&
      hbond_presentation.value().title=="Trajectory hydrogen-bond analysis" &&
  ...
  const auto ranged = trigger(
      gui, "traj range",
      {{"first", "0"}, {"last", "2"}, {"stride", "2"},
       {"mode", "loop"}, {"direction", "reverse"}});
  object = workspace->active_object();
  passed &= expect(
      ranged.succeeded() &&
  ...
  passed &= expect(
      object->trajectory->prefetch->snapshot().frame_indices ==
          std::vector<std::size_t>{0U, 2U},
      "range direction change must supersede prefetch with the new visit order");
  const auto speed = trigger(gui, "traj speed", {{"fps", "4"}});
  const auto started = trigger(
      gui, "traj play",
      {{"mode", "loop"}, {"direction", "reverse"}, {"steps", "0"}});
  const auto half_tick =
      trigger(gui, "traj tick", {{"elapsed-ms", "125"}});
  passed &= expect(
  ...
  const auto paused_tick =
      trigger(gui, "traj tick", {{"elapsed-ms", "1000"}});
  passed &= expect(
      paused.succeeded() && paused_tick.succeeded() &&
          !workspace->active_object()->trajectory->timeline.snapshot().playing &&
          workspace->active_object()->trajectory->timeline.snapshot().frame ==
              0U &&
          workspace->active_object()->trajectory->clock.pending_transitions() ==
              0.0,
      "pause must reset fractional time and ignore subsequent valid elapsed time");
  ...
  passed &= expect(
      trigger(gui, "traj range",
              {{"first", "0"}, {"last", "2"}, {"stride", "1"},
               {"mode", "loop"}, {"direction", "forward"}})
              .succeeded() &&
          trigger(gui, "traj speed", {{"fps", "1000"}}).succeeded() &&
          trigger(gui, "traj play", {{"mode", "loop"},
                                      {"direction", "forward"},
                                      {"steps", "0"}})
              .succeeded(),
  ...
  passed &= expect(
      catch_up.succeeded() &&
          ...
          workspace->active_object()->trajectory->clock.pending_transitions() ==
              880.0,
      "wall-clock catch-up must cap one tick at 120 transitions and retain backlog");

  passed &= expect(
      trigger(gui, "traj range",
              {{"first", "0"}, {"last", "2"}, {"stride", "1"},
               {"mode", "once"}, {"direction", "forward"}})
              .succeeded() &&
          trigger(gui, "traj speed", {{"fps", "30"}}).succeeded() &&
          trigger(gui, "traj play", {{"mode", "once"},
                                      {"direction", "forward"},
                                      {"steps", "0"}})
              .succeeded(),
  ...
  const auto failed_tick =
      trigger(gui, "traj tick", {{"elapsed-ms", "34"}});
  ...
  session.invocations = {
      ...
      {"traj load", {{"cache-mib", "1"},
                     {"file-format", "dcd"},
                     {"path", trajectory_path.string()}}},
      {"traj range", {{"direction", "reverse"},
                      {"first", "0"},
                      {"last", "2"},
                      {"mode", "loop"},
                      {"stride", "2"}}},
      {"traj speed", {{"fps", "4"}}},
      {"traj play", {{"direction", "reverse"},
                     {"mode", "loop"},
                     {"steps", "0"}}},
  ...
  const auto replayed = parsed.has_value()
                            ? application::replay_session(
                                  parsed.value(), replayed_dispatcher,
                                  replay_context)
  ...
  passed &= expect(
      serialized.has_value() && parsed.has_value() && replayed.has_value() &&
          ...
          replayed_workspace->active_object()->trajectory->timeline.snapshot()
                  .frame == 0U &&
          !replayed_workspace->active_object()
               ->trajectory->timeline.snapshot().playing &&
          replayed_workspace->active_object()
                  ->representations[0]
                  .packet.spheres[0]
                  .center.x == 0.0,
