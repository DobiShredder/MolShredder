...
int main() {
  ...
  const auto rotated =
      scene::rotate(quarter_turn, model::Vec3d{1.0, 0.0, 0.0});
  passed &= expect(near(rotated.x, 0.0) && near(rotated.y, 1.0),
                   "quaternion rotation must be right handed");

  scene::SceneBuilder builder;
  const auto group = builder.add_group(
      NodeId{0}, "protein group",
      Transform{{10.0, 0.0, 0.0}, {}, {1.0, 1.0, 1.0}});
  const auto system = builder.add_system(
      group.value(), "molecule", make_system(),
  ...
  passed &= expect(!builder.set_selection(
                        {system.value(), overlay.value(), system.value()})
                        .has_value() &&
  ...
  if (first.has_value()) {
    ...
    passed &= expect(molecule != nullptr &&
                         molecule->kind() == NodeKind::molecular_system &&
                         molecule->system() != nullptr &&
                         molecule->system()->id() == 77,
    ...
    passed &= expect(world.has_value() &&
                         scene::transform_point(world.value(), {}) ==
  ...
  passed &= expect(builder.reparent(overlay.value(), NodeId{0}, 2).has_value() &&
  ...
  passed &= expect(builder.set_transform(
                       system.value(),
  ...
  if (first.has_value()) {
    ...
    passed &= expect(!next.reparent(overlay.value(), group.value(), 0).has_value() &&
    ...
    passed &= expect(second.has_value() && second.value()->version() == 2 &&
