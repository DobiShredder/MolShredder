...
int main(int argc, char* argv[]) {
  ...
  passed &= expect(argc == 2,
                   "foundation grammar test requires one golden fixture");
  if (argc != 2) {
    return 1;
  }
  ...
  const std::vector<Invocation> inputs{
      ...
      {"select",
       {{"name", "protein"}, {"expression", "polymer.protein"}}},
  ...
  const auto invalid_representation = registry.normalize(
      Invocation{"show", {{"representation", "surface"}}});
  ...
  const auto invalid_precision = registry.normalize(
      Invocation{"analyze center", {{"precision", "16"}}});
  ...
  TaskContext context;
  const auto invoked = registry.invoke(
      Invocation{"com", {{"selection", "protein"}}}, context);
  passed &= expect(invoked.has_value(),
                   "registry invoke must normalize aliases and defaults");
  passed &= expect(invoked.has_value() &&
                       std::get<std::string>(
                           invoked.value().fields.at("mode").data) == "com" &&
                       std::get<std::string>(
                           invoked.value().fields.at("unit").data) ==
                           "angstrom",
